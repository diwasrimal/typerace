...
export function TypedWords({ typedWords }) {
  return <span className="typed-text">{typedWords.join(" ")}</span>;
}

export function NormalWords({ words }) {
  return <span className="unfocused-text">{words.join(" ")}</span>;
}
...
export function ActiveWord({ activeWord, activeLetterIdx }) {
  return (
    <>
      <span> </span>
      <span className="typed-text">{activeWord.slice(0, activeLetterIdx)}</span>
      <span className="active-letter">{activeWord[activeLetterIdx]}</span>
